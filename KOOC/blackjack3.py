...
class Deck(object):
    ...
    def __init__(self):
        ...
        random.shuffle(self.cards)  # cards 리스트 랜덤배열로

    def draw(self):
        """Draw the top card from the deck."""
        return self.cards.pop()
# --------------------------------------------------------------------
def create_clubs(symbol):  # 모양 만드는 함수
...
def create_diamonds(symbol):    # 모양만들기
...
def create_hearts(symbol):  # 모양만들기
...
def create_spades(symbol):  # 모양만들기
    circle1 = Circle(RADIUS, Point(-RADIUS, RADIUS))
    circle1.setFillColor('black')
    circle1.setBorderWidth(0)
    symbol.add(circle1)
